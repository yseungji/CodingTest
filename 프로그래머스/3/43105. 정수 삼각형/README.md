...
메모 : 계산횟수를 최적화하기 위해, 각 데이터에서 왼쪽과 오른쪽을 비교해 최댓값을 업데이트했다. 이때 제일 왼쪽 데이터는 if문으로 조건분기를 해주었지만, 제일 오른쪽 데이터는 놓쳐서 정확도에 오류가 있었다. 오른쪽도 조건분기를 해주어서 해결했다.
